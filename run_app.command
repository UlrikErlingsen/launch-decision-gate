...
PID_FILE=".venv/.gatesignal.pid"
PORT_FILE=".venv/.gatesignal.port"
...
if [ -f "$PID_FILE" ] && [ -f "$PORT_FILE" ]; then
  ...
  if /bin/kill -0 "$EXISTING_PID" 2>/dev/null && /usr/bin/curl -fsS "${EXISTING_URL}/_stcore/health" >/dev/null 2>&1; then
    echo "GateSignal is already running. Opening it now."
    if [ "${GATESIGNAL_NO_BROWSER:-0}" != "1" ]; then
...
if ! /usr/bin/env python3 -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)' 2>/dev/null; then
  echo "GateSignal needs Python 3.10 or newer."
...
if [ ! -d ".venv" ]; then
  echo "Creating GateSignal's private Python environment..."
...
READY_FILE=".venv/.gatesignal-requirements-${REQUIREMENTS_HASH}"
if [ ! -f "$READY_FILE" ]; then
  echo "First launch: downloading GateSignal's Python packages. Later launches will be faster."
  python -m pip --disable-pip-version-check install --prefer-binary -r requirements.txt
  /bin/rm -f .venv/.gatesignal-requirements-* .venv/.gatesignal-ready
  /usr/bin/touch "$READY_FILE"
else
  echo "Using the existing GateSignal environment."
fi

if [ -n "${GATESIGNAL_PORT:-}" ]; then
  PORT="$GATESIGNAL_PORT"
else
  PORT="$(python - <<'PY'
import socket

for candidate in [8597, *range(8501, 8600)]:
    sock = socket.socket()
    try:
...
echo "Starting GateSignal at ${URL}..."
...
while [ "$ATTEMPT" -le 120 ]; do
  if /usr/bin/curl -fsS "${URL}/_stcore/health" >/dev/null 2>&1; then
    echo "GateSignal is ready. Opening your browser..."
    if [ "${GATESIGNAL_NO_BROWSER:-0}" != "1" ]; then
      /usr/bin/open "$URL"
    fi
    wait "$APP_PID"
  ...
  if ! /bin/kill -0 "$APP_PID" 2>/dev/null; then
    echo "GateSignal stopped before it became ready. Review the message above."
    wait "$APP_PID"
    exit $?
  fi
  ATTEMPT=$((ATTEMPT + 1))
...
echo "GateSignal took too long to start. Review the message above, then try again."
exit 1
