...
if not exist .venv\.gatesignal-requirements-%REQ_HASH% (
  ...
  del /q .venv\.gatesignal-requirements-* .venv\.gatesignal-ready 2>nul
  type nul > .venv\.gatesignal-requirements-%REQ_HASH%
)
if not defined GATESIGNAL_PORT set GATESIGNAL_PORT=8597
python -m streamlit run app.py --server.headless=false --server.address=127.0.0.1 --server.port=%GATESIGNAL_PORT% --server.maxUploadSize=50 --server.fileWatcherType=none --browser.gatherUsageStats=false
